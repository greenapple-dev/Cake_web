 ...
 *
 *  Settings come from config.json when it exists (local use, e.g. on your own PC),
 *  and from environment variables otherwise (e.g. deployed on Render from GitHub,
 *  where config.json is intentionally not committed because it holds passwords).
 *  An environment variable always wins over config.json when both are set.
 *  See config.example.json and the README for the full list of variables.
 */
...
let fileCfg = {};
try { fileCfg = JSON.parse(fs.readFileSync(process.env.CAKE_CONFIG || path.join(__dirname, 'config.json'), 'utf8')); }
catch (e) { if (e.code !== 'ENOENT') throw e; /* no config.json – fine, e.g. on Render this all comes from env vars */ }

const pick = (...vals) => vals.find((v) => v !== undefined && v !== null && v !== '');
const envBool = (v, dflt) => (v === undefined ? dflt : /^(1|true|yes|on)$/i.test(v));
const onHost = !!(process.env.PORT || process.env.RENDER);   // true when clearly running on a hosting platform, not a local PC

const cfg = {
  serverName: pick(process.env.CAKE_SERVER_NAME, fileCfg.serverName, 'Cake SMP'),
  publicAddress: pick(process.env.CAKE_PUBLIC_ADDRESS, fileCfg.publicAddress, ''),
  host: pick(process.env.HOST, fileCfg.host, onHost ? '0.0.0.0' : '127.0.0.1'),
  port: pick(process.env.PORT, fileCfg.port, 8080),
  secureCookies: envBool(process.env.CAKE_SECURE_COOKIES, pick(fileCfg.secureCookies, onHost)),
  trustProxy: envBool(process.env.CAKE_TRUST_PROXY, pick(fileCfg.trustProxy, onHost)),
  storage: pick(process.env.CAKE_STORAGE, fileCfg.storage, 'local'),
  logFile: pick(process.env.CAKE_LOG_FILE, fileCfg.logFile, 'logs/latest.log'),
  minecraftDir: pick(process.env.CAKE_MINECRAFT_DIR, fileCfg.minecraftDir, '.'),
  rcon: {
    host: pick(process.env.CAKE_RCON_HOST, fileCfg.rcon && fileCfg.rcon.host, ''),
    port: Number(pick(process.env.CAKE_RCON_PORT, fileCfg.rcon && fileCfg.rcon.port, 25575)),
    password: pick(process.env.CAKE_RCON_PASSWORD, fileCfg.rcon && fileCfg.rcon.password, ''),
  },
  sftp: {
    host: pick(process.env.CAKE_SFTP_HOST, fileCfg.sftp && fileCfg.sftp.host, ''),
    port: Number(pick(process.env.CAKE_SFTP_PORT, fileCfg.sftp && fileCfg.sftp.port, 2022)),
    username: pick(process.env.CAKE_SFTP_USERNAME, fileCfg.sftp && fileCfg.sftp.username, ''),
    password: pick(process.env.CAKE_SFTP_PASSWORD, fileCfg.sftp && fileCfg.sftp.password, ''),
    root: pick(process.env.CAKE_SFTP_ROOT, fileCfg.sftp && fileCfg.sftp.root, '/'),
  },
  admin: {
    email: pick(process.env.CAKE_ADMIN_EMAIL, fileCfg.admin && fileCfg.admin.email, ''),
    passwordHash: pick(process.env.CAKE_ADMIN_PASSWORD_HASH, fileCfg.admin && fileCfg.admin.passwordHash, ''),
  },
  smtp: {
    host: pick(process.env.CAKE_SMTP_HOST, fileCfg.smtp && fileCfg.smtp.host, 'smtp.gmail.com'),
    port: Number(pick(process.env.CAKE_SMTP_PORT, fileCfg.smtp && fileCfg.smtp.port, 465)),
    secure: envBool(process.env.CAKE_SMTP_SECURE, pick(fileCfg.smtp && fileCfg.smtp.secure, true)),
    user: pick(process.env.CAKE_SMTP_USER, fileCfg.smtp && fileCfg.smtp.user, ''),
    pass: pick(process.env.CAKE_SMTP_PASS, fileCfg.smtp && fileCfg.smtp.pass, ''),
  },
  printCodeToConsole: envBool(process.env.CAKE_PRINT_CODE, pick(fileCfg.printCodeToConsole, false)),
  startCommand: pick(process.env.CAKE_START_COMMAND, fileCfg.startCommand, ''),
  restartCommand: pick(process.env.CAKE_RESTART_COMMAND, fileCfg.restartCommand, ''),
};
...
const SFTP_ROOT = (() => { let r = path.posix.normalize('/' + String(cfg.sftp.root || '/').replace(/\\/g, '/')); return r !== '/' ? r.replace(/\/+$/, '') : r; })();
...
(async () => {
  ...
  server.listen(PORT, HOST, () => {
    console.log(`\n  🍰  ${cfg.serverName || 'Cake SMP'} console → ${onHost ? 'listening on port ' + PORT : `http://localhost:${PORT}`}`);
    if (STORAGE === 'sftp') {
      ...
      if (!cfg.sftp.host || !cfg.sftp.username) console.log('      ! Set CAKE_SFTP_HOST / CAKE_SFTP_USERNAME / CAKE_SFTP_PASSWORD (Folium panel → Settings → Launch SFTP)');
    } else {
    ...
    if (!cfg.rcon.password) console.log('      ! Set CAKE_RCON_PASSWORD / rcon.password (same as rcon.password in server.properties)');
    if (!cfg.admin.email || !cfg.admin.passwordHash) console.log('      ! Set CAKE_ADMIN_EMAIL and CAKE_ADMIN_PASSWORD_HASH (run "npm run hash -- \\"your password\\"" to get the hash)');
    if (!cfg.printCodeToConsole && !cfg.smtp.pass) console.log('      ! Set CAKE_SMTP_PASS (a Gmail app password) or sign-in emails cannot be sent');
    if (HOST !== '127.0.0.1' && !cfg.secureCookies) console.log('      ! Exposed on the network without HTTPS – put it behind an HTTPS reverse proxy and set CAKE_SECURE_COOKIES=true');
    console.log('');
  });
})();
